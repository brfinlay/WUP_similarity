...
def similarity_iterations_50(list):
    sim_dict = {}
    for object in list:
...
x = similarity_iterations_50(all_objects)
...
df['Score'] = valuee
df.to_csv('/Users/brendafinlay/Documents/Cusack Lab/all_semantic_pairs_0.5.csv', index=False)

### ALL object pairs, no similarity threshold ###
...
y = similarity_iterations(all_objects)
kk = list(y.keys())
vv = list(y.values())
df = pd.DataFrame()
df['Objects'] = kk
df['Score'] = vv
df.to_csv('/Users/brendafinlay/Documents/Cusack Lab/all_semantic_pairs_no_threshold.csv', index=False)

##### Converting semantic data (y) to a matrix ######
import numpy as np

nobj = len(all_objects)
rdm = np.zeros([nobj, nobj])
for ind1, objname1 in enumerate(all_objects):
    for ind2, objname2 in enumerate(all_objects):
        rdm[ind1, ind2] = y[(objname1, objname2)]

pd.DataFrame(rdm).to_csv('/Users/brendafinlay/Documents/Cusack Lab/rsm.csv', index= all_objects)
